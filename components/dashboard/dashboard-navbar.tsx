...
export function DashboardNavbar() {
  ...
  return (
    <>
      {/* Header - Fixed structure with Logo and Profile */}
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-3rem)] max-w-7xl flex items-center justify-between pointer-events-none">
        {/* Left Logo - Floating Button Style */}
        <Link href="/" className="pointer-events-auto">
          <div className="flex items-center gap-3 px-4 py-2 rounded-2xl border border-white/10 bg-neutral-900/80 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] hover:border-white/20 transition-all group">
        ...
        {/* Profile Trigger - Floating on the right */}
        <div
          className="relative pointer-events-auto"
          onMouseEnter={() => setIsPopoverOpen(true)}
          onMouseLeave={() => setIsPopoverOpen(false)}
        >
          <button className="flex items-center gap-2 p-1 pr-4 rounded-2xl border border-white/10 bg-neutral-900/80 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] hover:border-white/20 transition-all overflow-hidden group">
          ...
          <AnimatePresence>
            {isPopoverOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-3 w-48 p-2 rounded-2xl border border-white/10 bg-neutral-900/90 backdrop-blur-2xl shadow-2xl overflow-hidden"
              >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-sky-500/10 text-sky-400 transition-colors group"
                >
